...
function startFunction() {
    if (BABYLON.Engine.isSupported()) {
      ...
      //Camera
      //var camera = new BABYLON.VirtualJoysticksCamera("VJC", new BABYLON.Vector3(-1800,230,3200), scene);
...
function download2ndfloor() {
  ...
  cmesh1.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPointerOverTrigger, function(ev){ 
        ...
        cmesh2.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPointerOverTrigger, function(ev){ 
  }));
...
function HandlePopup(objName) {
  ...
  if(is_popup_open == true) {
    ...
    if (BABYLON.Engine.isSupported()) {
      ...
      var canvas2 = document.getElementById("renderCanvas1");
        ...
        engine2.displayLoadingUI();
          ...
          /* manipulating custom product name */
          
          var custom_prod_names = [];
          custom_prod_names['mac1']='Mac Book Pro';
          custom_prod_names['mac2']='Mac Book Air';
          custom_prod_names['mac3']='Mac Book Retina';
          custom_prod_names['Sandal_5']='Red heel Sandal';
          custom_prod_names['shoe8']='Brown Leather Shoe-Right';
          custom_prod_names['shoe8v2']='Brown Leather Shoe-Left';
          custom_prod_names['tshirt12']='Sky Blue Casual T-shirt';
          custom_prod_names['tshirt13']='Sky Blue Sports T-shirt';
          document.getElementById('nametag').innerHTML = custom_prod_names[product_name];
          document.getElementById('buy_now_btnz').setAttribute("onclick", "buynow('"+product_name+"');");
          document.getElementById('add_tocart_btn').setAttribute("onclick", "addToCart('"+product_name+"');");
    }
  }  
}
    ...
    function buttoninc() {
        product_qty ++;
        document.getElementById('quant').value = product_qty;
    }
    function buttondec() {
        if (product_qty==1){
          product_qty=1;
...
function buynow(name){
  if(product_qty==1){
    ...
    total_qty_cart=total_qty_cart+product_qty;
  }
  //window.location.href = 'http://aghai.co.il/mage2/customcart?pids='+name+'&qtys='+product_qty;
}
...
document.getElementById('qty').value = total_qty_cart;
  myCart[name]=product_qty
  console.log('My Cart Qty are:'+myCart[name]);
